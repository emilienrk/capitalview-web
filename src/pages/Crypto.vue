...
<template>
  <div>
    ...
    <BaseCard v-if="crypto.currentAccount && selectedAccountId" class="mt-6" :padding="false">
      <template #header>
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 p-6 pb-0">
          <div>
          ...
          <table class="w-full text-sm">
            <thead>
              <tr class="text-left text-xs text-text-muted dark:text-text-dark-muted uppercase tracking-wider border-b border-surface-border dark:border-surface-dark-border">
                ...
                <th class="px-6 py-3 text-right">PRU</th>
                <th class="px-6 py-3 text-right">Investi</th>
                <th class="px-6 py-3 text-right">Cours</th>
            ...
            <tbody class="divide-y divide-surface-border dark:divide-surface-dark-border">
              <tr v-for="pos in crypto.currentAccount.positions" :key="pos.ticker" class="hover:bg-surface-hover dark:hover:bg-surface-dark-hover transition-colors">
                <td class="px-6 py-4 font-medium text-text-main dark:text-text-dark-main">{{ pos.ticker }}</td>
                <td class="px-6 py-4 text-right font-mono text-text-body dark:text-text-dark-body">{{ formatNumber(pos.total_amount, 6) }}</td>
                <td class="px-6 py-4 text-right">{{ formatCurrency(pos.average_buy_price) }}</td>
                <td class="px-6 py-4 text-right">{{ formatCurrency(pos.total_invested) }}</td>
                <td class="px-6 py-4 text-right">{{ formatCurrency(pos.current_price) }}</td>
                <td class="px-6 py-4 text-right font-medium">{{ formatCurrency(pos.current_value) }}</td>
          ...
          <table class="w-full text-sm">
            ...
            <tbody class="divide-y divide-surface-border dark:divide-surface-dark-border">
              <tr v-for="tx in accountTransactions" :key="tx.id" class="hover:bg-surface-hover dark:hover:bg-surface-dark-hover transition-colors">
                <td class="px-6 py-4 text-text-muted dark:text-text-dark-muted">{{ new Date(tx.executed_at).toLocaleDateString() }}</td>
                <td class="px-6 py-4">
                  <BaseBadge :variant="tx.type === 'BUY' ? 'success' : tx.type === 'SELL' ? 'danger' : 'warning'">
                    {{ tx.type }}
                  </BaseBadge>
                </td>
                <td class="px-6 py-4 font-medium text-text-main dark:text-text-dark-main">{{ tx.ticker }}</td>
                <td class="px-6 py-4 text-right font-mono">{{ formatNumber(tx.amount, 6) }}</td>
                <td class="px-6 py-4 text-right">{{ formatCurrency(tx.price_per_unit) }}</td>
                <td class="px-6 py-4 text-right font-medium">{{ formatCurrency(tx.amount * tx.price_per_unit) }}</td>
                <td class="px-6 py-4 text-right">
